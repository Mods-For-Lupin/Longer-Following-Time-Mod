...
import io.github.jason13official.stackable_stew_and_soup.impl.common.registry.ModMenus;
...
public class StackableStewAndSoupNeoForge {
  ...
  public StackableStewAndSoupNeoForge(final IEventBus modEventBus) {
    ...
    EVENT_BUS.addListener((Consumer<FMLCommonSetupEvent>) event -> StackableStewAndSoup.init());

    NeoForge.EVENT_BUS.addListener((Consumer<AddServerReloadListenersEvent>) event -> {
      event.addListener(StackableStewAndSoup.identifier(Constants.MOD_ID), new ResourceReloadListener());
    });

    if (FMLLoader.getCurrent().getDist() == Dist.CLIENT) {
      new StackableStewAndSoupClientNeoForge(EVENT_BUS);
    }
  }
  ...
  public static class ResourceReloadListener extends SimplePreparableReloadListener<Void> {
    ...
    public String getName() {
      return StackableStewAndSoup.identifier(Constants.MOD_ID).toString();
    }
